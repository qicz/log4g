...
func TestError(t *testing.T) {
	...
	assert.Error(t, e0, ".*/error_test.go:30 error")

	e1 := util.Errorf(code.FileLine(), "error: %d", 0)
	assert.Error(t, e1, ".*/error_test.go:33 error: 0")

	e2 := util.Wrap(e0, code.FileLine(), "error")
	assert.Error(t, e2, ".*/error_test.go:36 error; .*/error_test.go:30 error")

	e3 := util.Wrapf(e1, code.FileLine(), "error: %d", 1)
	assert.Error(t, e3, ".*/error_test.go:39 error: 1; .*/error_test.go:33 error: 0")
}
